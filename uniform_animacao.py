...
'''
Exemplo usando variavel uniform para translacao criando uma animação do objeto.
'''
...
# Nova posicao usada para transladar objeto
transladar_para_posicao = [ 0.5, 0.0, 0.0]
...
void main()
{
    vec3 new_pos = position + translation;
...
out vec4 FragColor;
uniform vec3 nova_cor;
void main()
{
    FragColor = vec4(nova_cor.r, nova_cor.g, nova_cor.b, 1.0f);
}
"""
...
def create_triangulo_vertices():
    ...
    '''
    lista_vertices = [
        [ -0.2,  -0.2,  0.0], # esquerda embaixo
        [ 0.2,  -0.2,  0.0], # direita embaixo
        [0.0,  0.2,  0.0] # centro cima
    ]
...
def refer_to_var_program( program_ref, var_in_program, var_data_type, vbo_ref):
    ...
    # Retorna uma referencia para variavel no shader ( com qualificador in ) 
...
def data_to_uniform(data_to_uniform, program_ref, var_in_program, var_data_type):
    """
    data_to_uniform :  Lista de valores float (e.g. cores ou posições dos vértices)
    """
    
    # Retorna uma referencia para variavel do tipo uniform no shader
    var_ref = gl.glGetUniformLocation(program_ref, var_in_program)

    if var_ref != -1:

        if var_data_type == "int":
            gl.glUniform1i(var_ref, data_to_uniform) # 1i significa um valor do tipo inteiro
            return data_to_uniform
        elif var_data_type == "bool":
            gl.glUniform1i(var_ref, data_to_uniform)
            return data_to_uniform
        elif var_data_type == "float":
            gl.glUniform1f(var_ref, data_to_uniform)
            return data_to_uniform
        elif var_data_type == "vec2":
            gl.glUniform2f(var_ref, data_to_uniform[0], data_to_uniform[1]) # 2f significa dois valores do tipo float
            return data_to_uniform
        elif var_data_type == "vec3":
            gl.glUniform3f(var_ref, data_to_uniform[0], data_to_uniform[1], data_to_uniform[2])
            return data_to_uniform
        elif var_data_type == "vec4":
            gl.glUniform4f(var_ref, data_to_uniform[0], data_to_uniform[1], data_to_uniform[2], data_to_uniform[3])
            return data_to_uniform
        elif var_data_type == "mat4":
            gl.glUniformMatrix4fv(var_ref, 1, gl.GL_TRUE, data_to_uniform)
            return data_to_uniform
    else:
        return None
...
def draw_triangulo(shaderProgramRef, uniform_pos=None, uniform_color=None):
    '''
    '''
    ...
    if (uniform_pos != None):
        transladar_para_posicao = data_to_uniform(data_to_uniform=uniform_pos, program_ref=shaderProgramRef, var_in_program='translation', var_data_type='vec3')
        new_values = valores_para_animacao(transladar_para_posicao)
        data_to_uniform(data_to_uniform=new_values, program_ref=shaderProgramRef, var_in_program='translation', var_data_type='vec3')
    else:
        posicao_padrao = [0.0, 0.0, 0.0]
        data_to_uniform(data_to_uniform=posicao_padrao, program_ref=shaderProgramRef, var_in_program='translation', var_data_type='vec3')

    if (uniform_color != None):
        values_from_translation = data_to_uniform(data_to_uniform=uniform_color, program_ref=shaderProgramRef, var_in_program='nova_cor', var_data_type='vec3')
    else:
        cor_padrao = [1.0, 0.0, 0.0]
        values_from_translation = data_to_uniform(data_to_uniform=cor_padrao, program_ref=shaderProgramRef, var_in_program='nova_cor', var_data_type='vec3')

    gl.glDrawArrays(gl.GL_TRIANGLES, 0, quant_vert)

    return VertexArrayObject, quant_vert

def valores_para_animacao(in_values=None):

    # Retorna o tempo decorrido desde da chamada da funcao glutInit
    tempo = glut.glutGet(glut.GLUT_ELAPSED_TIME)/1000

    if (in_values != None):
        in_values[0] += 0.001
        if in_values[0] > 1.2: # valor limite
            in_values[0] = -1.2
        return in_values 
    else:
        # Retorna somente tempo decorrido
        return tempo
    ...
    for i in range(2):
        for j in range(2):
            for k in range(2):
                ...
                print(j)
                print(k)
                if(a == 0.0 and b == 0.0 and c == 1.0 or a == 1.0 and b == 1.0 and c == 0.0 or a == 1.0 and b == 0.0 and c == 0.0 or a == 0.0 and b == 1.0 and c == 0.0 or a == 1.0 and b == 0.0 and c == 0.0  ):
                    gl.glClearColor(0.5, 0.5, 0.5, 1.0)
                    gl.glClear(gl.GL_COLOR_BUFFER_BIT)  
                    
                    mudar_cor = [a,b,c]
                    draw_triangulo(shaderProgramRef=shaderProgramRef, uniform_pos=transladar_para_posicao, uniform_color=mudar_cor)

                glut.glutSwapBuffers()
                # Redesenha conteúdo da janela para mostrar a nova posição do objeto.
...
def main_opengl(titulo_janela):
    ...
    init_window(titulo_janela, 512, 512)
...
if __name__ == '__main__':
    ...
    titulo_janela = 'UNIFORM ANIMACAO'
    print("\n")
    print(titulo_janela)
    print("\n")
